...
async def get_books(
    ...
    books = await get_books_from_db(title, author, genre, published_date, description, image_url, pdf_url)
    return books


@router.get('/{book_id}', response_model=Book, summary='Returns book data')
async def get_book(book_id: int):
    result = await get_book_from_db(book_id)
    if result is None:
...
async def delete_book(book_id: int, user_data: User = Depends(get_current_user)):
    book = await delete_book_from_db(book_id)
    if book is None:
        raise HTTPException(status_code=404, detail="Book not found")
    return book
