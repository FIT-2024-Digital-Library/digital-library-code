...
class ReviewsCrud(CrudInterface):
    ...
    async def get(cls, session: AsyncSession, element_id: int):
        query = select(review_table.c[cls.REVIEW_FIELDS]).where(review_table.c.id == element_id)
        result = await session.execute(query)
        return result.mappings().first()

    @classmethod
    async def get_multiple(cls, session: AsyncSession, filters: ReviewsFiltersScheme = None):
        query = (select(*[review_table.c[field] for field in cls.REVIEW_FIELDS])
        ...
        result = await session.execute(query)
        return [review for review in result.mappings().all()]

    @classmethod
    async def create(cls, session: AsyncSession, model: ReviewCreate, owner_id: int = None):
        ...
        result = await session.execute(
            ...
            .returning(review_table.c[cls.REVIEW_FIELDS])
        )
        ...
        await BooksCrud.update(session, model.book_id, BookUpdate(avg_mark=new_avg, marks_count=new_reviews_count))
        await session.commit()
        return Review(**result.mappings().first())

    @classmethod
    async def delete(cls, session: AsyncSession, element_id: int, owner_id: int = None):
        ...
        reviews_count_for_book = book['marks_count']
        new_reviews_count = reviews_count_for_book - 1
        new_avg = (current_avg * reviews_count_for_book - review[
            'mark']) / new_reviews_count if new_reviews_count > 0 else 0

        await BooksCrud.update(session, review['book_id'],
                               BookUpdate(avg_mark=new_avg, marks_count=new_reviews_count))
        await session.execute(delete(review_table).where(review_table.c.id == element_id))
        await session.commit()
        return review

    @classmethod
    async def update(cls, session: AsyncSession, element_id: int, owner_id: int, model: ReviewUpdate = None):
        ...
        if model.mark is not None and model.mark != old_review['mark']:
            ...
            current_avg = book['avg_mark']
            reviews_count_for_book = book['marks_count']
            new_avg = (current_avg * reviews_count_for_book - old_review['mark'] + model.mark) / reviews_count_for_book
